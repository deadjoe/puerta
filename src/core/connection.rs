...
impl ConnectionManager {
    ...
    pub async fn accept_connection(&self, listener: &TcpListener) -> Result<(TcpStream, SocketAddr), io::Error> {
        ...
        match timeout(self.backend_timeout, TcpStream::connect(backend_addr)).await {
            ...
            Ok(Err(e)) => {
                match e.kind() {
                    ...
                    _ => {
                        debug!("Network error connecting to backend {}: {}", backend_addr, e);
    ...
    pub async fn forward_data(
        ...
        info!(
            ...
            client_addr, backend_addr, stats.connection_duration_ms,
            stats.bytes_client_to_backend, stats.bytes_backend_to_client
        );
...
mod tests {
    ...
    async fn test_backend_connection_refused() {
        ...
        let result = manager.connect_to_backend("127.0.0.1:65534".parse().unwrap()).await;
    ...
    async fn test_backend_connection_timeout() {
        ...
        let result = manager.connect_to_backend("10.255.255.1:9999".parse().unwrap()).await;
    ...
    async fn test_data_forwarding() {
        ...
        let result = manager.forward_data(client_stream, backend_stream, client_addr, backend_addr).await;
